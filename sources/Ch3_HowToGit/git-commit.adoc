...
ちなみに、コミットメッセージなどが表示されている場所の「ラベル」部分に「HEAD」という記載がありますが、これは「現時点でどのブランチにいるかを判別する情報」です。つまり、世界線の観測ができているという事で、例えるならダイバージェンスメーターです。
...
GitHub for Windows (Mac) の場合「ステージングエリア」にファイルを移動する必要はなく、いきなり GitHub for Windows (Mac) の画面下部でコミットメッセージを書くことができます。
